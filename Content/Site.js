...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,WebSharper,Seq,Operators,d3,Math,IntrinsicFunctionProxy,Site,Samples,List,Circles,Set,FocusBrushing,CompaniesGraph,UI,WorldTour,Client,jQuery,Data,Concurrency,Arrays,Html,Default,T,Operators1,Data1,D3Extras,Error,Number,Builder,Sample,document,PageletExtensions,Unchecked,EventsPervasives;
 Runtime.Define(Global,{
  Site:{
   Circles:{
    ...
    {
     d3.select(context).append("svg").attr("height",500).selectAll("circle").data(data).enter().append("circle").attr("cx",function(x)
     {
      return x*5;
     }).attr("cy",function(x)
     {
     ...
     {
      return 7+5*Math.sin(3*(+tupledArg[1]/+IntrinsicFunctionProxy.GetLength(data))*3.14159265358979);
     }));
    },
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("Circles").FileName("Circles.fs").Keywords(List.ofArray(["basic"])).Render(function(context)
     {
      return Circles.Join(Circles.Data(),context);
     }).Create();
    })
   },
   Client:{
    All:Runtime.Field(function()
    {
     ...
     return Set.Create(List.ofArray([_op_BangPlus_9(Circles.Sample()),_op_BangPlus_9(FocusBrushing.Sample()),_op_BangPlus_9(UI.Sample()),_op_BangPlus_9(WorldTour.Sample())]));
    }),
   ...
   CompaniesGraph:{
    Data:{
     FileName:Runtime.Field(function()
     {
      return"CompaniesGraph.json";
     })
    },
    UI:{
     Data:{
     ...
     {
      var height,width,array,x,forceNodes,x1,forceLinks,tick,force,svg,node1,link;
      ...
      forceNodes=Arrays.sortBy(function(node)
      {
       return node.index;
      },x);
      ...
      svg=d3.select(config.Parent).append("svg").attr("class","CompaniesGraph").attr("width",width).attr("height",height);
      node1=svg.selectAll(".node").data(forceNodes).enter().append("circle").attr("class","node").call(force.drag).on("mouseover",Runtime.Tupled(function(tupledArg)
      {
     ...
     Sample:Runtime.Field(function()
     {
      return Samples.Build().Id("CompaniesGraph").FileName("CompaniesGraph.UI.fs").Keywords(List.ofArray(["force"])).Render(function(ctx)
      {
       return UI.Show(ctx);
      }).Create();
     }),
     ...
     {
      var label,x,main;
      label=Default.Div(Runtime.New(T,{
       $:0
      }));
      x=Default.Div(List.ofArray([Default.Span(List.ofArray([Default.Text("In Focus: ")])),label]));
      Operators1.OnAfterRender(function(parent)
      {
       return UI.Start(parent.Body,label);
      },x);
      main=x;
      ctx.appendChild(main.Body);
      return main.Render();
     },
     Start:function(parent,out)
     {
      return Concurrency.Start(Concurrency.Delay(function()
      {
       ...
       {
        return Concurrency.Return(UI.Render({
         CanvasHeight:500,
         CanvasWidth:960,
         DataSet:arg101,
         OnMouseOver:function(label)
         {
          return out.set_Text(label.$==1?label.$0:label.$0+" (revenue: "+Global.String(label.$1)+")");
         },
         Parent:parent,
         Radius:function(label)
         {
          return label.$==1?4:label.$1===0?4:Operators.Max(4,Math.sqrt(label.$1)/30000);
         }
        }));
       });
      }));
   ...
   D3Extras:{
    "D3.Json.Static":function(url)
    {
     ...
     {
      return Global.d3.json(url,function(x,y)
      {
       return c(x,y);
      });
     });
    ...
    "D3.Tsv.Static":function(url)
    {
     return D3Extras.Defer(function(c)
     {
      return Global.d3.tsv(url,function(x,y)
      {
       return c(x,y);
      });
     });
    ...
    Defer:function(job)
    {
     ...
     callback=Runtime.Tupled(function(tupledArg)
     {
      ...
      ok=tupledArg[0];
      no=tupledArg[1];
      return job(Runtime.Tupled(function(tupledArg1)
      {
       var data;
       data=tupledArg1[1];
       return tupledArg1[0]?no(new Error("Defer: failed")):ok(data);
      }));
     });
     return Concurrency.FromContinuations(function(ok)
     {
   ...
   FocusBrushing:{
    ...
    {
     var x,w,margin,margin2,width,height,height2,x1,x2,y,y2,xAxis,xAxis2,yAxis,brush,area,area2,svg,focus,context;
     x=jQuery(canvas).width();
     w=Number(x);
     margin={
      Top:10,
      Right:10,
     ...
     svg=d3.select(canvas).append("svg").attr("class","FocusBrushing").attr("height",height+margin.Top+margin.Bottom);
     ...
     {
      x1.domain(brush.empty()?x2.domain():(Runtime.Tupled(function(x3)
      {
       return x3;
      }))(brush.extent()));
     ...
     return d3.csv("sp500.csv",function(data)
     {
      ...
      x1.domain((Runtime.Tupled(function(x3)
      {
       return x3;
    ...
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("FocusBrushing").FileName("FocusBrushing.fs").Render(function(canvas)
     {
   ...
   Samples:{
    ...
    Sample:Runtime.Class({
     Show:function()
     {
      ...
      url="http://github.com/intellifactory/websharper.d3/blob/master/Site/"+this.FileName;
   ...
   WorldTour:{
    Render:function(ctx)
    {
     var projection,title,canvas,c,path;
     projection=d3.geo.orthographic().scale(248).clipAngle(90);
     title=d3.select(ctx).append("h1");
     canvas=d3.select(ctx).append("canvas").attr("width",960).attr("height",500);
     ...
     return Concurrency.Start(Concurrency.Delay(function()
     {
      ...
      x=D3Extras["D3.Json.Static"]("WorldTour/world-110m.json");
      return Concurrency.Bind(x,function(arg101)
      {
       var x1;
       x1=D3Extras["D3.Tsv.Static"]("WorldTour/world-country-names.tsv");
       return Concurrency.Bind(x1,function(arg102)
       {
        var landFeature,countries,borders,projection1,array,x2,transition;
        landFeature=WorldTour.topojson().feature(arg101,arg101.objects.land);
        countries=WorldTour.topojson().feature(arg101,arg101.objects.countries).features;
        borders=WorldTour.topojson().mesh(arg101,arg101.objects.countries,Runtime.Tupled(function(tupledArg)
        {
         return tupledArg[0]!==tupledArg[1];
        }));
        projection1=function(c1)
        {
         return c1.name;
        };
        array=Arrays.filter(function(d)
        {
         return Seq.exists(function(n)
         {
          if(d.id==n.id)
           {
            d.name=n.name;
        ...
        x2=Arrays.sortBy(projection1,array);
        transition=function(i)
        {
         d3.transition().duration(1250).each("start",Runtime.Tupled(function()
         {
          title.text(x2[i].name);
         })).tween("rotate",function()
         {
          var patternInput,p1,p0,r;
          patternInput=d3.geo.centroid(x2[i]);
          p1=patternInput[1];
          p0=patternInput[0];
          r=d3.interpolate(projection.rotate(),[-p0,-p1,0]);
          return function(t)
          {
           var patternInput1,z,y,x3;
           patternInput1=r(t);
           z=patternInput1[2];
           y=patternInput1[1];
           x3=patternInput1[0];
           projection.rotate([x3,y,z]);
         ...
         }).transition().each("end",Runtime.Tupled(function()
         {
          return transition(i+1);
         }));
        };
        return Concurrency.Return(transition(0));
       });
      });
     }));
    },
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("WorldTour").FileName("WorldTour.fs").Render(function(ctx)
     {
      return WorldTour.Render(ctx);
     }).Create();
    }),
 ...
 Runtime.OnInit(function()
 {
  ...
  CompaniesGraph=Runtime.Safe(Site.CompaniesGraph);
  UI=Runtime.Safe(CompaniesGraph.UI);
  WorldTour=Runtime.Safe(Site.WorldTour);
  Client=Runtime.Safe(Site.Client);
  jQuery=Runtime.Safe(Global.jQuery);
  Data=Runtime.Safe(CompaniesGraph.Data);
  Concurrency=Runtime.Safe(WebSharper.Concurrency);
  Arrays=Runtime.Safe(WebSharper.Arrays);
  Html=Runtime.Safe(WebSharper.Html);
  Default=Runtime.Safe(Html.Default);
  ...
  Data1=Runtime.Safe(UI.Data);
  D3Extras=Runtime.Safe(Site.D3Extras);
  Error=Runtime.Safe(Global.Error);
  Number=Runtime.Safe(Global.Number);
